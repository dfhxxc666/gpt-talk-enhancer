...
const entryFile = resolve(projectRoot, "src/extension-entry.js");
...
await mkdir(resolve(outputDirectory, "main"), { recursive: true });
await writeFile(resolve(outputDirectory, "content.js"), bundle, "utf8");
await copyFile(resolve(projectRoot, "extension/manifest.json"), resolve(outputDirectory, "manifest.json"));
await copyFile(resolve(projectRoot, "extension/main/chatgpt-api-capture.js"), resolve(outputDirectory, "main/chatgpt-api-capture.js"));
await copyFile(resolve(projectRoot, "extension/NOTICE.md"), resolve(outputDirectory, "NOTICE.md"));
await copyFile(resolve(projectRoot, "extension/THIRD_PARTY_GPL-3.0.txt"), resolve(outputDirectory, "THIRD_PARTY_GPL-3.0.txt"));
console.log(`Built ${relative(projectRoot, outputDirectory)} (${modules.size} modules, ${bundle.length} bytes content bundle)`);
...
async function collectModule(filePath) {
  ...
  for (const dependency of imports) await collectModule(dependency);
